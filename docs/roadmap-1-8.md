...
| 3 | 通知・リマインダー | **要** | 設計のみ |
| 4 | 教科カバレッジ（情報Ⅰ・政経・物理・数学・古文） | 不要（basic型） | 着手済み |
| 5 | 過去問タグ運用 | 不要（tags列） | 規約を策定済み |
| 6 | 志望校データに配点を追加 | **要**（assets差し替え） | 京大分のデータ確定 |
| 7 | 自動バックアップ | **要** | 設計のみ |
...
検証済みの値は `data/universities/kyodai_haiten.tsv` に確定させた。
出典は令和9年度 京都大学 募集要項（`S70RM3892/-` の `data/r9.txt` に座標抽出済み、縦計1025で検算済み）。
...
二次素点650（国100/数200/理200/英150）→ 工学部配点800 への換算は
